...
def calculate_similarity(seq_1, seq_2):
    """
    function to calculate the similarity of sequences
    :param seq_1:  str, one sequence of MSA
    :param seq_2:  str, one sequence of MSA
    :return: similarity: float, the similarity of different sequences
    """
    numerator = 0
    denominator = 0
...
def buildSimilarityMatrix(seqs_lst):
    """
    function to make similarity matrix
    :param seqs_lst: list of str
    :return: matrix: matrix, similarity of matrix
    """
...
def buildDistanceMatrix(matrix):
    ...
    :return: distance matrix: distance matrix
    """
...
def buildHierarchicalCluster(distance_matrix):
    """
    function to make hierarchical cluster
    :param distance_matrix: matrix
    :return: linkage_matrix: matrix, linkage matrix for hierarchical cluster
    """
...
def cutHierarchicalCluster(linkage_matrix):
    """
    function to cut the hierarchical tree
    :param linkage_matrix: matrix, linkage matrix for hierarchical cluster
    :return:
    cuttree: matrix
    dict_seq: dictionary, key is the index of clusters, values is the index of sequences
    """
    cuttree = cut_tree(linkage_matrix, height=0.01)
    list_same = []
    for i in cuttree:
...
def main():
    ...
    names_list, species_list, sequences_list = parse_fasta_file('../real_data_pfam/aux_aux_b3_auxin/aux_b3_auxin_exact.fasta')
    ...
    write_fasta_file('../real_data_pfam/aux_aux_b3_auxin/aux_b3_auxin_exact_cut0.01.fasta', new_seqs, new_names)
